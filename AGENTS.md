...
- **Admin (unificado):** `AdminApp` con prop `level` cubre las 3 plantillas (`basic` → `BasicSections` sin tabs; `standard`/`premium` → shells con tabs y `DashboardTab`/`StatsTab`/`KitchenTab`/`POSTab` en `packages/blocks/admin`). Las páginas admin de cada template son wrappers con login guard.
